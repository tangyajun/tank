...
public class GamersSmallTankFactory implements WeaponFactory<GamersTank> {
	...
	@Override
	public GamersTank createWeapon(int x,int y,Dir dir,  Group group) {
		return new GamersTank(x,y,dir,group,new SmallTankFireBulletStrategy());
	}
	 ...
	 */
	@Override
	public List<GamersTank> createWeapons(int num, Group group, final int distance, Dir dir,int x,int y) {
	...
	@Override
	public List<GamersTank> createWeapons(int num,Group group, final int distance,Dir dir) {
		List<GamersTank> tanks=new LinkedList<>();
		int x=140;
		int y=40;
		for (int i=0;i<num;i++) {
